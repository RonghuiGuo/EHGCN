...
class LastFM(BasicDataset):
    ...
    def getSparseGraph(self, load_adj, keep_prob, Hadj):
        if self.Graph is None and load_adj == 'H_adj':
            try:
                if Hadj == 1:
                    self.Graph = torch.load(self.path + '/s_pre_adj_mat_HyperGraph1_keepprob{}.pt'.format(keep_prob))
                    self.Graph = self.Graph.coalesce().to(device)
                    print('load H_adj in one-hop done')           
                    return self.Graph
                if Hadj == 2:
                    self.Graph = torch.load(self.path + '/s_pre_adj_mat_HyperGraph2_keepprob{}.pt'.format(keep_prob))
                    self.Graph = self.Graph.coalesce().to(device)
                ...
                if Hadj == 1:
                    ...
                    if keep_prob != 1:
                        A_u = drop_scipy_matrix(A_u, keep_prob=keep_prob)
                        A_i = drop_scipy_matrix(A_i, keep_prob=keep_prob)
                    ...
                    torch.save(self.Graph, self.path + '/s_pre_adj_mat_HyperGraph1_keepprob{}.pt'.format(keep_prob))
                    print('save norm_adj in' + self.path + '/s_pre_adj_mat_HyperGraph1_keepprob{}.pt'.format(keep_prob))
                    self.Graph = self.Graph.coalesce().to(device)
                    return self.Graph
                if Hadj == 2:
                ## hypergraph 2
                    ...
                    torch.save(self.Graph, self.path + '/s_pre_adj_mat_HyperGraph2_keepprob{}.pt'.format(keep_prob))
                    print('save norm_adj in' + self.path + '/s_pre_adj_mat_HyperGraph2_keepprob{}.pt'.format(keep_prob))
                    self.Graph = self.Graph.coalesce().to(device)
                    return self.Graph
...
class newLoader(BasicDataset):
    def __init__(self,path="../data/AMusic"):
    ...
    def getSparseGraph(self, keep_prob, Hadj, load_adj):
            ...
            try:
                if load_adj == 'H_adj':
                    ...
                    if Hadj == 1:
                        ...
                        H_pre_adj_mat = sp.load_npz(self.path + '/s_pre_adj_mat_HyperGraph1_keepprob{}.npz'.format(keep_prob))
                    if Hadj == 2:
                        H_pre_adj_mat = sp.load_npz(self.path + '/s_pre_adj_mat_HyperGraph2_keepprob{}.npz'.format(keep_prob))
                    print('====>H_adj exist...')
                ...
                if load_adj == 'H_adj':
                    ...
                    if Hadj == 1:
                        ...
                        if keep_prob != 1:
                            R_u = drop_scipy_matrix(R_u, keep_prob=keep_prob)
                            R_i = drop_scipy_matrix(R_i, keep_prob=keep_prob)
                        ...
                        sp.save_npz(self.path + '/s_pre_adj_mat_HyperGraph1_keepprob{}.npz'.format(keep_prob), H_norm_adj)
                        print('save norm_adj in' + self.path + '/s_pre_adj_mat_HyperGraph1_keepprob{}.npz'.format(keep_prob))
                        return
                    if Hadj == 2:
                        print("generate Hadj in two-hop")
                        R = self.UserItemNet.tolil() 
                        ...
                        print(f"costing {end-s}s, saved norm_mat...")
                        sp.save_npz(self.path + '/s_pre_adj_mat_HyperGraph2_keepprob{}.npz'.format(keep_prob), H_norm_adj)
                        print('save norm_adj in' + self.path + '/s_pre_adj_mat_HyperGraph2_keepprob{}.npz'.format(keep_prob))
                        return
    def __build_test(self):
        """
        return:
...
if __name__ == "__main__":
    dataset = newLoader(path="../data/AToy")
    # dataset.getSparseGraph(load_adj="H_adj", Hadj=1, keep_prob=0.1)
    # dataset.getSparseGraph(load_adj="H_adj", Hadj=1, keep_prob=0.2)
    # dataset.getSparseGraph(load_adj="H_adj", Hadj=1, keep_prob=0.3)
    # dataset.getSparseGraph(load_adj="H_adj", Hadj=1, keep_prob=0.4)
    # dataset.getSparseGraph(load_adj="H_adj", Hadj=1, keep_prob=0.5)
    # dataset.getSparseGraph(load_adj="H_adj", Hadj=1, keep_prob=0.6)
    # dataset.getSparseGraph(load_adj="H_adj", Hadj=1, keep_prob=0.7)
    # dataset.getSparseGraph(load_adj="H_adj", Hadj=1, keep_prob=0.8)
    # dataset.getSparseGraph(load_adj="H_adj", Hadj=1, keep_prob=0.9)
    # dataset.getSparseGraph(load_adj="H_adj", Hadj=1, keep_prob=1)


    dataset.getSparseGraph(load_adj="H_adj", Hadj=2, keep_prob=0.1)
    dataset.getSparseGraph(load_adj="H_adj", Hadj=2, keep_prob=0.2)
    dataset.getSparseGraph(load_adj="H_adj", Hadj=2, keep_prob=0.3)
    dataset.getSparseGraph(load_adj="H_adj", Hadj=2, keep_prob=0.4)
    dataset.getSparseGraph(load_adj="H_adj", Hadj=2, keep_prob=0.5)
    dataset.getSparseGraph(load_adj="H_adj", Hadj=2, keep_prob=0.6)
    dataset.getSparseGraph(load_adj="H_adj", Hadj=2, keep_prob=0.7)
    dataset.getSparseGraph(load_adj="H_adj", Hadj=2, keep_prob=0.8)
    dataset.getSparseGraph(load_adj="H_adj", Hadj=2, keep_prob=0.9)
    dataset.getSparseGraph(load_adj="H_adj", Hadj=2, keep_prob=1)
