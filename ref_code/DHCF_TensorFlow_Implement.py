...
class DHCF(DeepRecommender):
    ...
    def initModel(self):
        ...
        user_embeddings = self.U                    # 用户嵌入  E_u
        item_embeddings = self.V                    # 物品嵌入  E_i
        ...
        for i in range(self.n_layer):
            new_user_embeddings = tf.sparse_tensor_dense_matmul(H_u, self.U)        # 用户嵌入  # H_u * E_u
            new_item_embeddings = tf.sparse_tensor_dense_matmul(H_i, self.V)        # 物品嵌入  # H_i * E_i
